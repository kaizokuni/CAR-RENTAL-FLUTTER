...
func Seed() {
	...
	if err != nil {
		log.Printf("⚠️  Failed to create/get admin tenant: %v", err)
		return
	}

	log.Printf("✅ Admin Tenant ID: %s", tenantID)

	// 2. Provision Admin Tenant DB
	err = database.CreateTenantDB(adminDBName)
	if err != nil {
		log.Printf("ℹ️  Database might already exist: %v", err)
	}

	err = database.MigrateTenantDB(adminDBName)
	if err != nil {
		log.Printf("⚠️  Failed to migrate admin tenant DB: %v", err)
		return
	}
	...
	if err != nil {
		log.Printf("⚠️  Failed to connect to admin tenant DB: %v", err)
		return
	}
	...
	if err != nil {
		log.Printf("⚠️  Failed to sync tenant record: %v", err)
		return
	}
	...
	if err != nil {
		log.Printf("⚠️  Failed to create admin role: %v", err)
		return
	}
	...
	if err != nil {
		log.Printf("⚠️  Failed to create super admin user: %v", err)
		return
	}

	log.Println("🎉 Super Admin Seeding Complete!")
	log.Printf("   URL:      http://%s.localhost:5173", adminSubdomain)
